...
def main():
    ...
    with col1:
        if st.button("👟 신발"):
            st.session_state.messages.append({"role": "user", "content": "신발 수선 상담받고 싶어요"})
            st.session_state.messages.append({"role": "MARU", "content": st.session_state.bot.get_response("신발")})
            st.experimental_rerun()
    with col2:
        if st.button("👜 가방"):
            st.session_state.messages.append({"role": "user", "content": "가방 수선 문의드려요"})
            st.session_state.messages.append({"role": "MARU", "content": st.session_state.bot.get_response("가방")})
            st.experimental_rerun()
    with col3:
        if st.button("👔 옷"):
            st.session_state.messages.append({"role": "user", "content": "옷 수선 가능한가요"})
            st.session_state.messages.append({"role": "MARU", "content": st.session_state.bot.get_response("옷")})
            st.experimental_rerun()
    with col4:
        if st.button("💰 가격"):
            st.session_state.messages.append({"role": "user", "content": "수선 가격이 궁금해요"})
            st.session_state.messages.append({"role": "MARU", "content": st.session_state.bot.get_response("가격")})
            st.experimental_rerun()
